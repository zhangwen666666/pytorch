...
def save_car(car_samples, data_root_dir, data_annotation_dir, data_jpeg_dir):
    ...
    保存类别Car的样本图片和标注文件
    """
    for sample_name in car_samples:  # 图片名
        # ../../data/VOCdevkit/VOC2007/Annotations/图片名.xml
        src_annotation_path = os.path.join(voc_annotation_dir, sample_name + suffix_xml)
        # ../../data/voc_car/train/Annotations/图片名.xml
...
if __name__ == '__main__':
    ...
    # train：car_train.txt中标签为1的样本
    # val：car_val.txt中标签为1的样本
    print(samples)
    # samples = sample_train_val(samples)
    # print(samples)

    # 检测'../../data/voc_car/'是否存在
    check_dir(car_root_dir)
    for name in ['train', 'val']:
        data_root_dir = os.path.join(car_root_dir, name)  # ../../data/voc_car/train
        data_annotation_dir = os.path.join(data_root_dir, 'Annotations')  # ../../data/voc_car/train/Annotations
        data_jpeg_dir = os.path.join(data_root_dir, 'JPEGImages')  # ../../data/voc_car/train/JPEGImages

        check_dir(data_root_dir)
        check_dir(data_annotation_dir)
        check_dir(data_jpeg_dir)
        # 将../../data/VOCdevkit/VOC2007/Annotations/路径下的标签为1的.xml文件保存到
        # ../../data/voc_car/train(val)/Annotations/文件夹下
