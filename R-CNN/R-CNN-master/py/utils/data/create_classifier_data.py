...
from py.utils.util import check_dir, parse_car_csv, parse_xml, compute_ious, iou
...
if __name__ == '__main__':
    ...
    gs = selectivesearch.get_selective_search()  # 获取一个segmentation对象
    for name in ['train', 'val']:
        src_root_dir = os.path.join(car_root_dir, name)  # ../../data/voc_car/train(val)
        src_annotation_dir = os.path.join(src_root_dir, 'Annotations')  # ../../data/voc_car/train(val)/Annotations
        src_jpeg_dir = os.path.join(src_root_dir, 'JPEGImages')  # ../../data/voc_car/train(val)/JPEGImages

        dst_root_dir = os.path.join(classifier_root_dir, name)  # ../../data/classifier_car/train(val)
        dst_annotation_dir = os.path.join(dst_root_dir,
                                          'Annotations')  # ../../data/classifier_car/train(val)/Annotations
        dst_jpeg_dir = os.path.join(dst_root_dir, 'JPEGImages')  # ../../data/classifier_car/train(val)/JPEGImages
