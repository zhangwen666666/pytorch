...
class CustomBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(self.num_iter):
            tmp = np.concatenate(   #连接两个或多个数组
                (random.sample(self.idx_list[:self.num_positive], self.batch_positive), #从self.idx_list[:self.num_positive]中随机采样self.batch_positive个样本
                 random.sample(self.idx_list[self.num_positive:], self.batch_negative))
            )
            random.shuffle(tmp) #随机打乱
