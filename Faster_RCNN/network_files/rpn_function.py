...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    """
    box_cls_flattened = []  # 存储预测目标分数
    box_regression_flattened = []   # 存储bounding box回归的参数

    # 遍历每个预测特征层
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        # [batch_size, anchors_num_per_position * classes_num, height, width]
        # 注意，当计算RPN中的proposal时，classes_num=1,只区分目标和背景   A表示anchor
        ...
        # [N, -1, C] N表示batch_size,C表示类别,-1表示的是该张图片(当前特征层上)上的anchor总数
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        """
        筛除小boxes框(面积小)，nms处理，根据预测概率获取前post_nms_top_n个目标
        Args:
            proposals: 预测的bbox坐标
            objectness: 预测的目标概率
    ...
    def forward(self,
        ...
        # 计算anchor传入网络之后的预测类别和坐标偏移量
        # objectness[batch,A*C,H,W]（A表示anchor个数,C=1,只用来区分前景和背景）
        # pred_bbox_deltas [batch,A*4,H,W]（A*4表示的是每个anchor的4个位置偏移信息）
        ...
        # 调整内部tensor格式以及shape
        # 调整之后objectness[batch*当前特征层上anchor总数，1]
        # pred_bbox_deltas[batch*当前特征层上anchor总数，4]
        ...
        if self.training:
            ...
            # 在labels中，正样本被标记为1，负样本被标记为0，被丢弃的样本被标记为-1
